...
const Projects = ({ isDesktop, clientHeight }) => {
  ...
  useEffect(() => {
    ...
    if (isDesktop) {
      ...
      const projectWrapper =
        sectionRevealRef.current.querySelector(".project-wrapper");
  ...
  }, [sectionRevealRef, sectionTitleRef, isDesktop]);

  const getRevealSt = () => {
    const revealTl = gsap.timeline({ defaults: { ease: "none" } });
    revealTl.from(
      sectionRevealRef.current.querySelectorAll(".staggered-reveal"),
  ...
  const getProjectsSt = () => {
    ...
    const sidePadding =
      ...
      sectionRevealRef.current.querySelector(".inner-container").clientWidth;
    const elementWidth =
      sidePadding +
      sectionRevealRef.current.querySelector(".project-wrapper").clientWidth;
    sectionRevealRef.current.style.width = `${elementWidth}px`;
    const width = window.innerWidth - elementWidth;
    const duration = `${(elementWidth / window.innerHeight) * 100}%`;
    timeline
      .to(sectionRevealRef.current, { x: width })
